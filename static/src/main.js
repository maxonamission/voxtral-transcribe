...
/**
 * Detect the insertion context based on preceding text in the DOM.
 * Delegates string analysis to the shared detectContext().
 */
function detectInsertContextFromNode(node) {
    const before = getTextBefore(node);
    if (!before) return "sentence-start";
    // Extract the last line (shared detectContext works on a single line)
...
function isAfterSentenceEnd(node) {
    return detectInsertContextFromNode(node) !== "mid-sentence";
}
...
// ── Queue (imported from queue.js) ──
// saveToQueue, getQueueCount, processQueue imported at top
